    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct UserInfo {
        uint256 allocation; // amount taken into account to obtain RAM (amount spent + discount)
        uint256 contribution; // amount spent to buy RAM
    ...
    IERC20Upgradeable public RAM_TOKEN; // Project token contract
    ...
    uint256 public MAX_RAM_TOKENS_TO_DISTRIBUTE; // max RAM amount to distribute during the auction
    uint256 public MIN_TOTAL_RAISED_FOR_MAX_RAM_TOKEN; // amount to reach to distribute max RAM amount [should be uncapped, always distribute max at the end]
    ...
    function initialize(
        ...
        END_TIME = endTime;
        treasury = treasury_;
        MAX_RAM_TOKENS_TO_DISTRIBUTE = maxToDistribute;
        MIN_TOTAL_RAISED_FOR_MAX_RAM_TOKEN = minToRaise;
        if (maxToRaise == 0) {
        ...
        weth = IWETH(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1); // arb1weth address
     ...
     *
     * Will be marked as inactive if RAM_TOKEN has not been deposited into the contract
     */
    ...
    /**
     * @dev Check whether users can claim their purchased RAM_TOKEN
     *
    ...
    /**
     * @dev Returns the amount of RAM_TOKEN to be distributed based on the current total raised
     */
    function tokensToDistribute() public view returns (uint256) {
        if (MIN_TOTAL_RAISED_FOR_MAX_RAM_TOKEN > totalRaised) {
            return
                (MAX_RAM_TOKENS_TO_DISTRIBUTE * totalRaised) /
                MIN_TOTAL_RAISED_FOR_MAX_RAM_TOKEN;
        }
        return MAX_RAM_TOKENS_TO_DISTRIBUTE;
    }

    /**
     * @dev Get user share times 1e5
     */
    function getExpectedClaimAmount(address account)
        public
        view
        returns (uint256)
    {
    ...
    function buyETH() external payable isAuctionActive nonReentrant {
        require(
            address(COLLATERAL_TOKEN) == address(weth),
            "error: collateral must be ETH"
        );
    ...
    /**
     * @dev Purchase an allocation for the auction for a value of "amount" AUCTION_TOKEN,"
     */
    ...
    function _buy(uint256 amount) internal {
        ...
        if (user.cap == 0) {
            ...
            require(
                user.contribution + amount <= user.cap,
                "buy: wallet cap reached"
            );
        }
        ...
        if (
            ...
            allocation =
                allocation +
                ((discountEligibleAmount * user.discount) / 100);
        }
    ...
    /**
     * @dev Claim purchased RAM_TOKEN during the auction
     */
    ...
    /**
     * @dev Assign custom discounts, used for Monolith migrators
     *
     * Based on saved moSOLID amounts from the migration contract
     * Suggested to give 5% discount, with a cap of 10,000 tokens
     */
    function setUsersDiscount(DiscountSettings[] calldata users)
        public
        onlyOwner
    {
    ...
    function setRAMToken(address ramToken) external onlyOwner {
        RAM_TOKEN = IERC20Upgradeable(ramToken);
    }

    function setUsersCap(address[] memory users, uint256 cap)
        external
        onlyOwner
    {
    ...
    /** @dev function for users to claim their RAM from moSOLID migration on Ethereum
     *
     *
     *   function claimMigrationAllocation() public
     *    {
     *    // require msg.sender has allocation, claim their allocation to their wallet
     *    }
     *
     *   function forceClaimOnBehalf(address _migratedAddress, address _claimForWallet) public onlyOwner
     *   {
     *   // require _migratedAddress has a claim, send token to the _claimForWallet
     *   }
     */
    ...
    function _safeClaimTransfer(address to, uint256 amount) internal {
        uint256 balance = RAM_TOKEN.balanceOf(address(this));
        bool transferSuccess = false;

        if (amount > balance) {
            transferSuccess = RAM_TOKEN.transfer(to, balance);
        } else {
            transferSuccess = RAM_TOKEN.transfer(to, amount);
        }
